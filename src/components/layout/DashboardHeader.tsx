...
export function DashboardHeader({ onMenuToggle }: { onMenuToggle?: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-4 border-b border-slate-200 bg-white px-4 md:px-8">
      ...
      <div className="flex items-center gap-4">
        <form className="hidden md:block" onSubmit={onSubmit}>
          <div className="flex w-[280px] items-center gap-2 rounded-[10px] border border-slate-200 bg-slate-100 px-3 py-2.5">
        ...
        <button
          ...
          className="relative inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:bg-slate-50 hover:text-slate-700"
          type="button"
        >
          <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden text-right sm:block">
            ...
            <span className="mt-1 inline-flex rounded-md bg-indigo-50 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-indigo-500">
              {user?.role ?? 'ADMIN'}
            </span>
          </div>
        </div>
